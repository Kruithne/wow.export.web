...
const SIGMA_32: number[] = [0x61707865, 0x3320646e, 0x79622d32, 0x6b206574];
const SIGMA_16: number[] = [0x61707865, 0x3120646e, 0x79622d36, 0x6b206574];

export default class Salsa20 {
	private rounds: number;
	private sigma: number[];
	private keyWords: number[];
	private nonceWords: number[];
	 ...
	 * @param nonce 8 byte nonce array
	 * @param key 16 or 32 byte hex string key
	 * @param rounds Number of rounds, defaults to 20
	 */
	constructor(nonce: number[] | Uint8Array, key: string, rounds: number = 20) {
		if (nonce.length !== 8)
	 ...
	 * @returns BufferReader with generated bytes
	 */
	getBytes(byteCount: number): BufferReader {
		const out = BufferReader.alloc(byteCount);
	 ...
	 * @param buf Input buffer to process
	 * @returns Processed buffer
	 */
	process(buf: BufferReader): BufferReader {
		const out = BufferReader.alloc(buf.byteLength);
